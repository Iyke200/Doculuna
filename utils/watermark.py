...
try:
    ...
    raise ImportError("PyMuPDF (fitz) required for PDF handling")

# Reportlab for image-to-PDF
...
try:
    ...
    logger.warning("reportlab not available; image-to-PDF conversion disabled")
...
class WatermarkManager:
    ...
    async def add_text_watermark(
        ...
        try:
            ...
            processing_time = time.time() - start_time
            input_type_str = getattr(file_path, 'suffix', type(file_path).__name__) if isinstance(file_path, (str, Path)) else type(file_path).__name__
    ...
    async def add_logo_watermark(
        self,
        file_path: Union[str, Path, bytes, BytesIO],
        logo_path: Union[str, Path, bytes, BytesIO],
        config: Optional[WatermarkConfig] = None,
        output_format: str = '.png'
    ) -> bytes:
        """
    ...
    async def add_watermark(
        ...
        try:
            ...
            if watermark_type in [WatermarkType.TEXT, WatermarkType.BOTH] and config.text:
                watermarked_bytes_io = BytesIO(watermarked)
                watermarked_bytes_io.seek(0)
                watermarked = await self.add_text_watermark(
                    watermarked_bytes_io,
                    config.text,
    ...
    async def batch_watermark(
        ...
        try:
            ...
            async def process_single_file(file_info):
                async with semaphore:
                    ...
                    try:
                        ...
                        output_path = None
                        if output_dir:
                            output_filename = Path(file_path).name.rsplit('.', 1)[0] + '_watermarked.' + file_type.lstrip('.')
            ...
            logger.info("Batch watermark completed", extra={
                ...
                'total_size_in_mb': round(total_size_in / (1024*1024), 2) if total_size_in else 0,
                'total_size_out_mb': round(total_size_out / (1024*1024), 2) if total_size_out else 0,
                'batch_time_s': round(batch_time, 3),
                'avg_time_per_file_s': round(batch_time / len(final_results), 3) if final_results else 0
            })
    ...
    async def _load_input(self, file_input: Union[str, Path, bytes, BytesIO]) -> Tuple[bytes, str]:
        ...
        try:
            ...
            if isinstance(file_input, (str, Path)):
                ...
                try:
                    ...
                    file_type = '.' + img.format.lower() if img.format else '.png'
                except:
                    file_type = '.pdf'  # Assume PDF if not image
    ...
    async def _load_logo(self, logo_input: Union[str, Path, bytes, BytesIO]) -> Optional[bytes]:
        ...
        try:
            ...
            elif isinstance(logo_input, BytesIO):
                logo_data = logo_input.read()
            else:
    ...
    async def _add_text_to_image(
        ...
        try:
            ...
            # Estimate size first for position
            bbox = draw.textbbox((0, 0), text, font=font)
            text_width = bbox[2] - bbox[0]
            text_height = bbox[3] - bbox[1]
            position = self._calculate_position(image.size, config, (text_width, text_height))
            
            # Draw text with transparency
            self._draw_transparent_text(image, draw, text, position, font, config)
    ...
    async def _add_text_to_pdf(
        ...
        try:
            ...
            # Process each page
            page_range = config.page_range or list(range(len(doc)))
            for page_num in page_range:
                ...
                # Add text (note: rotation not supported in insert_text; use shape for advanced)
                page.insert_text(
                    point,
                    text,
    ...
    async def _add_logo_to_image(
        ...
        try:
            ...
            # Resize logo
            logo_size = self._calculate_logo_size(base_image.size, logo_image.size, config)
            ...
            # Calculate position
            position = self._calculate_position(base_image.size, config, logo_size)
            
            # Paste logo
            base_image.paste(logo_image, (int(position[0]), int(position[1])), logo_image)
    ...
    async def _add_logo_to_pdf(
        ...
        try:
            ...
            # Load logo as image to get size
            ...
            for page_num in page_range:
                ...
                # Calculate position and size for PDF
                point = self._calculate_pdf_position(rect, config)
                logo_size = self._calculate_logo_size((int(rect.width), int(rect.height)), logo_image.size, config)
                
                # Create rect for insertion
                insert_rect = fitz.Rect(point[0], point[1], point[0] + logo_size[0], point[1] + logo_size[1])
                ...
                # Embed using modern PyMuPDF syntax
                img_doc = fitz.open(stream=logo_bytes, filetype="png")
                page.show_pdf_page(insert_rect, img_doc, pno=0)
                img_doc.close()
        ...
        try:
            if source_format in self.SUPPORTED_IMAGE_FORMATS and target_format == '.pdf':
                if REPORTLAB_AVAILABLE:
                    ...
                    raise WatermarkError("reportlab not available for image-to-PDF conversion")
    ...
    async def _get_font(self, config: WatermarkConfig) -> ImageFont.ImageFont:
    ...
    def _calculate_position(self, image_size: Tuple[int, int], config: WatermarkConfig, 
                            watermark_size: Tuple[int, int] = (0, 0)) -> Tuple[float, float]:
        ...
        try:
            ...
            elif x_ratio > 0.5:  # Right
                x = min(width - config.padding - w_width, x)
            else:  # Center
                x = max(config.padding, min(width - config.padding - w_width, x))
            
            if y_ratio < 0.5:  # Top
                y = max(config.padding, y)
            elif y_ratio > 0.5:  # Bottom
                y = min(height - config.padding - w_height, y)
            else:  # Center
                y = max(config.padding, min(height - config.padding - w_height, y))
    ...
    def _calculate_logo_size(self, image_size: Tuple[int, int], 
                             original_logo_size: Tuple[int, int], 
                             config: WatermarkConfig) -> Tuple[int, int]:
        """Calculate optimal logo size."""
        try:
            width, height = image_size
            orig_width, orig_height = original_logo_size
            max_width = width * config.scale
            max_height = height * config.scale
            
            # Preserve aspect ratio
            ratio = min(max_width / orig_width, max_height / orig_height)
            logo_width = int(orig_width * ratio)
            logo_height = int(orig_height * ratio)
    ...
    def _draw_transparent_text(
        ...
        try:
            # Calculate text size (use (0,0) for bbox to avoid position influence)
            bbox = draw.textbbox((0, 0), text, font=font)
            text_width = bbox[2] - bbox[0]
            text_height = bbox[3] - bbox[1]
            
            # Update position with actual size
            adjusted_pos = self._calculate_position(image.size, config, (text_width, text_height))
            
            # Create transparent background for better readability
            if config.font_color != (0, 0, 0):  # Not black
                # Draw semi-transparent black background
                bg_x1 = adjusted_pos[0] - 5
                bg_y1 = adjusted_pos[1] - 5
                bg_x2 = bg_x1 + text_width + 10
                bg_y2 = bg_y1 + text_height + 10
                draw.rectangle([bg_x1, bg_y1, bg_x2, bg_y2], fill=(0, 0, 0, int(128 * config.opacity)))
            
            # Draw rotated text
            if config.rotation != 0:
                # Create temporary image for rotation
                temp_size = (int(text_width + 20), int(text_height + 20))
                temp_image = Image.new('RGBA', temp_size, (0, 0, 0, 0))
                ...
                if rot_bbox:
                    ...
                    paste_x = int(adjusted_pos[0] - rot_width / 2)
                    paste_y = int(adjusted_pos[1] - rot_height / 2)
                    
                    image.paste(rotated, (paste_x, paste_y), rotated)
            else:
                # Draw directly
                draw.text(adjusted_pos, text, fill=config.font_color, font=font)
                
        except Exception as e:
            logger.error("Text drawing failed", extra={'error': str(e)})
            # Fallback: draw simple text
            fallback_pos = (position[0], position[1])
            draw.text(fallback_pos, text, fill=(255, 255, 255), font=ImageFont.load_default())
...
async def add_text_watermark(file_input: Union[str, Path, bytes, BytesIO], text: str, **kwargs) -> bytes:
...
async def add_logo_watermark(file_input: Union[str, Path, bytes, BytesIO], logo_input: Union[str, Path, bytes, BytesIO], **kwargs) -> bytes:
...
async def batch_watermark(files: List[Union[str, Path, Tuple[str, bytes]]], config: WatermarkConfig, **kwargs) -> List[Dict[str, Any]]:
